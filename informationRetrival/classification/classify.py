...
class classify(object):
    def train_data(self):
        ...
        query_results = collection.find({$or: [{$and: [{"genre.name":"Horror"},{$or: [{"overview": {$regex: /horr/i}},{"overview": {$regex: /evil/i}},{"overview": {$regex: /blood/i}}]} ]}, {$and: [{"genre.name":"Romance"},{$or: [{"overview": {$regex: /roman/i}},{"overview": {$regex: /love/i}}]} ]}, {$and: [{"genre.name":"Crime"},{$or: [{"overview": {$regex: /rob/i}},{"overview": {$regex: /murder/i}}]} ]}]}).distinct().count()
        dict = []
        categ = []
        train = []
